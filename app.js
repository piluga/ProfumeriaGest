...
function initDB() {
    return new Promise((resolve, reject) => {
        ...
        // 🔥 FIX: Lettura corretta del messaggio di errore moderno
        request.onerror = function (event) {
            reject("Dettaglio: " + (event.target.error ? event.target.error.message : "Accesso negato dal browser"));
        };
    });
}
...
window.chiudiModaleAvviso = function () {
    let testoAvviso = (document.getElementById('msg-avviso').innerText || "").toUpperCase();

    // Identifichiamo subito di che tipo di avviso si tratta
    let isClienteNonTrovato = testoAvviso.includes("NESSUN CLIENTE TROVATO");
    let isProdottoMancante = testoAvviso.includes("FORMATO NON DISPONIBILE");
    let modaleBlocco = document.getElementById('modal-sblocco-cassa');

    if (isProdottoMancante) {
        // 🔥 FIX FLICKERING: Apriamo il modale delle linee PRIMA di chiudere l'avviso
        if (typeof apriModale === "function") apriModale('modal-essenze');
        else document.getElementById('modal-essenze').style.display = 'block';

        // Impostiamo direttamente la vista sui formati
        let stepCat = document.getElementById('step-essenza-categoria');
        let stepFormato = document.getElementById('step-essenza-formato');
        if (stepCat) stepCat.style.display = 'none';
        if (stepFormato) stepFormato.style.display = 'block';

        // Ora chiudiamo l'avviso in modo pulito senza mostrare la cassa sotto
        chiudiModale('modal-avviso');
    } else {
        // Per tutti gli altri avvisi, chiudiamo prima il modale normalmente
        chiudiModale('modal-avviso');

        if (modaleBlocco && modaleBlocco.style.display !== 'none') {
            setTimeout(() => document.getElementById('input-pin-sblocco').focus(), 100);
        } else if (isClienteNonTrovato) {
            setTimeout(() => {
                let campoScheda = document.getElementById('campo-scheda');
                if (campoScheda) {
                    campoScheda.focus();
                    campoScheda.select();
                }
...
function aggiungiProdotto(prodotto) {
    ...
    if (itemInCart) {
        ...
        nuovaRiga.className = 'riga-prodotto';
        nuovaRiga.id = 'riga-' + prodotto.codice;
        nuovaRiga.title = "Un clic: Aggiungi 1 | Doppio clic: Rimuovi 1";
		...
		// --- 🖱️ GESTIONE ROTELLINA DEL MOUSE SULLA RIGA ---
        nuovaRiga.addEventListener('wheel', function (e) {
            e.preventDefault(); // Evita lo scorrimento della pagina
            
            let index = carrello.findIndex(i => i.codice === prodotto.codice);
            if (index > -1) {
                let item = carrello[index];
                
                if (e.deltaY < 0) {
                    // Rotellina SU: Aumenta la quantità
                    item.qta++;
                    totaleLordo += item.prezzo;
                } else if (e.deltaY > 0) {
                    // Rotellina GIÙ: Diminuisce (si ferma a 1 per evitare cancellazioni involontarie)
                    if (item.qta > 1) {
                        item.qta--;
                        totaleLordo -= item.prezzo;
                    }
                ...
                // Aggiorna istantaneamente i testi solo su questa riga
                this.querySelector('.qta-val').textContent = item.qta;
                this.querySelector('.tot-riga-val').textContent = '€ ' + (item.qta * item.prezzo).toLocaleString('it-IT', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
        ...
        let clickTimer = null;
        nuovaRiga.addEventListener('click', function (e) {
            let rigaCorrente = this;

            if (clickTimer === null) {
                // --- SINGOLO CLIC: AGGIUNGE 1 PEZZO ---
                // Avvia un timer di 250ms per aspettare un eventuale secondo clic
                clickTimer = setTimeout(() => {
                    ...
                    if (index > -1) {
                        ...
                        item.qta++;
                        totaleLordo += item.prezzo;
                        rigaCorrente.querySelector('.qta-val').textContent = item.qta;
                        rigaCorrente.querySelector('.tot-riga-val').textContent = '€ ' + (item.qta * item.prezzo).toLocaleString('it-IT', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
                        if (barraCliente.style.display !== 'flex') { mostraMessaggio("AGGIUNTO 1 PEZZO: " + item.descrizione); }
                        aggiornaSchermo(); aggiornaContatori();
                        if (document.getElementById('modal-numpad').style.display !== 'flex') { campoBarcode.focus(); }
                    }
                }, 250);
            } else {
                // --- DOPPIO CLIC: RIMUOVE 1 PEZZO O ELIMINA ---
                // Il secondo clic è arrivato in tempo, quindi annulla l'aggiunta e scala il pezzo!
                clearTimeout(clickTimer);
                clickTimer = null;

                let indexDaEliminare = carrello.findIndex(i => i.codice === prodotto.codice);
                if (indexDaEliminare > -1) {
                    let item = carrello[indexDaEliminare];
                    totaleLordo -= item.prezzo;
                    if (totaleLordo < 0.01) totaleLordo = 0;

                    if (item.qta > 1) {
                        item.qta--;
                        rigaCorrente.querySelector('.qta-val').textContent = item.qta;
                        rigaCorrente.querySelector('.tot-riga-val').textContent = '€ ' + (item.qta * item.prezzo).toLocaleString('it-IT', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
                        if (barraCliente.style.display !== 'flex') { mostraMessaggio("RIMOSSO 1 PEZZO: " + item.descrizione); }
                    } else {
                        carrello.splice(indexDaEliminare, 1);
                        rigaCorrente.remove();
                        if (barraCliente.style.display !== 'flex') { mostraMessaggio("ARTICOLO RIMOSSO: " + item.descrizione); }
                    }
                    aggiornaSchermo(); aggiornaContatori();
                    if (document.getElementById('modal-numpad').style.display !== 'flex') { campoBarcode.focus(); }
                }
...
if (btnCassa) {
    btnCassa.addEventListener('click', function () {
        ...
        // 1.5. Controllo Sicurezza Resi/GiftCard (Blocca i codici dinamici VOU e GC)
        let haArticoloFisico = carrello.some(item => {
            let code = String(item.codice || "").toUpperCase();
            let desc = String(item.descrizione || "").toUpperCase();

            // Riconosce se la riga è un Buono Reso, una Gift Card o uno Sconto
            let isVoucher = code.startsWith('VOU');
            let isGiftCard = code.startsWith('GC');
            let isSconto = code === 'SCONTO' || desc.includes('SCONTO') || desc.includes('BUONO');

            // Se non è nessuna di queste cose, significa che c'è almeno un prodotto vero e proprio
            return !isVoucher && !isGiftCard && !isSconto;
        });

        if (!haArticoloFisico) {
            mostraAvvisoModale("⚠️ <b>OPERAZIONE NEGATA</b><br><br>Non puoi chiudere uno scontrino contenente solo un Buono Reso o una Gift Card.<br>Aggiungi almeno un articolo fisico da acquistare prima di procedere.");
            return;
        }
        ...
        if (typeof clienteAttivo !== 'undefined' && clienteAttivo && clienteAttivo.bonus > 0 && totaleNettoAttuale > 0) {
...
window.confermaVendita = async function (riscattaBonus) {
    ...
    if (clienteAttivo) {
        ...
        // --- LOGICA VISIVA INTELLIGENTE PER I PUNTI ---
        let segnoPunti = puntiAcquisiti > 0 ? '+' : '';
        let colorePunti = puntiAcquisiti > 0 ? '#00cc66' : '#ff4d4d'; // Verde se in positivo, Rosso se è un reso
        let etichettaPunti = puntiAcquisiti > 0 ? 'Punti guadagnati:' : 'Punti stornati:';
        ...
        if (riscattaBonus) {
            ...
            messaggioEsito = `Scontrino emesso riscattando il bonus.<br><br>Punti spesi: <b style="color:#ff6666;">⭐ -${puntiSpesi}</b><br>${etichettaPunti} <b style="color:${colorePunti};">⭐ ${segnoPunti}${puntiString}</b><br><br>Nuovo saldo punti: <b>⭐ ${clienteAttivo.punti.toLocaleString('it-IT', { maximumFractionDigits: 2 })}</b>`;
        } else {
            clienteAttivo.punti += puntiAcquisiti;
            clienteAttivo.bonus = Math.floor(clienteAttivo.punti / 100) * 10;
            clienteAttivo.dataUltimaOperazione = dataDiOggiStr;
            saldoFinale = clienteAttivo.punti;
            messaggioEsito = `Scontrino emesso.<br><br>${etichettaPunti} <b style="color:${colorePunti};">⭐ ${segnoPunti}${puntiString}</b><br><br>Nuovo saldo punti: <b>⭐ ${clienteAttivo.punti.toLocaleString('it-IT', { maximumFractionDigits: 2 })}</b>`;
        }
        ...
        // 🔥 DECIDE QUALE TEMPLATE USARE IN BASE AL BONUS FINALE DELLO SCONTRINO 🔥
        let templateMsg = "";

        // Il bonus in questa funzione è già un numero, possiamo controllarlo direttamente!
        if (clienteAttivo.bonus < 10) {
            templateMsg = localStorage.getItem('impostazioni_msg_vendita_min') || MSG_VENDITA_MINORE_DEFAULT;
        } else {
            templateMsg = localStorage.getItem('impostazioni_msg_vendita_max') || MSG_VENDITA_MAGGIORE_DEFAULT;
        }
	...
	// 🔥 ESTRAZIONE IMPORTI PER IL REGISTRATORE FISCALE
    let importoContantiPerCassa = tipoPagamento.toUpperCase() === "CONTANTI" ? pagato : (tipoPagamento === "Misto" ? importoMistoContanti : 0);
    let importoPosPerCassa = tipoPagamento.toUpperCase() === "POS" ? pagato : (tipoPagamento === "Misto" ? importoMistoPos : 0);

    // 🔥 INVIO SCONTRINO FISCALE AL REGISTRATORE (Con Scudo di Sicurezza)
    let contieneReso = carrello.some(item => (item.prezzo * item.qta) < 0 || item.is_reso || item.categoria === "VOUCHER");
    let contieneGiftCard = carrello.some(item => item.is_giftcard || item.is_giftcard_ricarica);

    if (contieneReso) {
        console.warn("🖨️ Stampa Fiscale annullata: Il carrello contiene un reso o un voucher. Procedere con storno manuale sulla Cassa.");
    } else if (contieneGiftCard) {
        console.warn("🖨️ Stampa Fiscale annullata: Il carrello contiene una Gift Card (Fuori Campo IVA). Gestire l'operazione manualmente.");
    } else {
        // Se è una vendita valida, invia lo scontrino passando anche la divisione Contanti/POS!
        inviaScontrinoFiscale(carrello, totaleNettoAttuale, bonusUsato, importoContantiPerCassa, importoPosPerCassa); 
    }
    ...
    // 🛡️ SALA D'ATTESA: Evita il blocco del database locale
    let prodottiDaAggiornareCloud = [];
    let giftCardDaAggiornareCloud = [];
    let voucherDaEliminareCloud = [];
    ...
    for (let item of carrello) {
                    ...
                    if (reqComp.result) {
                        ...
                        if (compDB.formato === 'ml' || compDB.inci) {
                            ...
                            window.etichetteCheckout.push({
                                nome_profumo: item.descrizione, formato_ml: comp.qta, prezzo: item.prezzo, barcode: item.codice, inci: compDB.inci || "INCI NON INSERITO", lotto: lottoPrincipale, pao: paoReale, copie: item.qta
                            });
                        }

                        compDB.timestamp_sync = Date.now();
                        storeMag.put(compDB);
                        prodottiDaAggiornareCloud.push(compDB); // 🛡️ Mette in attesa invece di inviare
                    }
                }
            });
        }
        ...
        if (item.categoria === "VOUCHER") {
            storeVou.delete(item.codice);
            voucherDaEliminareCloud.push(item.codice); // 🛡️ Mette in attesa
            ...
            reqGC.onsuccess = function () {
                if (reqGC.result) {
                    ...
                    gc.storico.push({ data: getOggiString(), ora: `${String(new Date().getHours()).padStart(2, '0')}:${String(new Date().getMinutes()).padStart(2, '0')}`, importo: -Math.abs(item.prezzo), tipo: "ACQUISTO IN CASSA" });
                    if (gc.saldo <= 0.01) { gc.saldo = 0; gc.stato = "ESAURITA"; }
                    storeGC.put(gc);
                    giftCardDaAggiornareCloud.push(gc); // 🛡️ Mette in attesa
                }
            };
            ...
            reqGC.onsuccess = function () {
                if (reqGC.result) {
                    ...
                    if (!gc.storico) gc.storico = [];
                    gc.storico.push({ data: getOggiString(), ora: `${String(new Date().getHours()).padStart(2, '0')}:${String(new Date().getMinutes()).padStart(2, '0')}`, importo: Math.abs(item.prezzo), tipo: "RICARICA CREDITO" });
                    storeGC.put(gc);
                    giftCardDaAggiornareCloud.push(gc); // 🛡️ Mette in attesa
                }
            };
            ...
            req.onsuccess = function () {
                if (req.result) {
                    req.result.giacenza += item.qta;
                    req.result.timestamp_sync = Date.now();
                    storeMag.put(req.result);
                    prodottiDaAggiornareCloud.push(req.result); // 🛡️ Mette in attesa
                }
            }
            ...
            req.onsuccess = function () {
                if (req.result) {
                    ...
                    if (!prodotto.is_kit || prodotto.tipo_kit !== 'DINAMICO') {
                        prodotto.giacenza -= item.qta;
                        prodotto.timestamp_sync = Date.now();
                        storeMag.put(prodotto);
                        prodottiDaAggiornareCloud.push(prodotto); // 🛡️ Mette in attesa
                    }
                }
            }
        }
    ...
    // 🛡️ SINCRONIZZAZIONE SICURA CLOUD POST-TRANSAZIONE
    // Scatta in automatico solo quando il PC ha terminato i salvataggi locali
    txMag.oncomplete = function () {
        prodottiDaAggiornareCloud.forEach(p => { if (typeof salvaProdottoCloud === "function") salvaProdottoCloud(p); });
        giftCardDaAggiornareCloud.forEach(gc => { if (typeof salvaGiftCardCloud === "function") salvaGiftCardCloud(gc); });
        voucherDaEliminareCloud.forEach(v => { if (typeof eliminaVoucherCloud === "function") eliminaVoucherCloud(v); });

        // 🔥 AGGIORNAMENTO GRAFICO ISTANTANEO DEL MAGAZZINO IN BACKGROUND
        if (typeof magCaricaLista === "function") {
            magCaricaLista();
        }
    };

    txMag.onerror = function (event) {
        if (typeof mostraAvvisoModale === "function") {
            mostraAvvisoModale("Errore durante il salvataggio locale. La transazione è stata interrotta.");
        }
    };
...
campoBarcode.addEventListener('keydown', async function (e) {
    ...
    if (listaRicerca.style.display === 'none' || elementi.length === 0) {
        if (e.key === 'Enter') {
            e.preventDefault();
            // Forza il codice cercato in maiuscolo
            const codice = this.value.trim().toUpperCase();

            const magazzinoGrezzo = await getAll('magazzino');
            const magazzinoCompleto = calcolaGiacenzeVirtualiBOM(magazzinoGrezzo);

            // Cerca convertendo anche i codici del database in maiuscolo per un match perfetto
            const prodotto = magazzinoCompleto.find(p => String(p.codice).toUpperCase() === codice);
...
document.querySelectorAll('.opt-sconto').forEach(opt => {
    opt.addEventListener('click', function () {
        ...
        // 🔥 FIX 2: Nasconde il menu padre per forzare la chiusura CSS e sposta il focus
        let menuPadre = this.parentElement;
        if (menuPadre) {
            menuPadre.style.display = 'none';
            setTimeout(() => {
                menuPadre.style.display = '';
                document.getElementById('campo-barcode').focus();
            }, 50);
        }
    });
});
...
document.querySelectorAll('.opt-pagamento').forEach(opt => {
    opt.addEventListener('click', function () {
        campoPagamento.value = this.textContent;
        barraDestra.textContent = this.getAttribute('data-icona');
...
window.inviaSaldoPuntiManuale = function () {
    ...
    if (numero.length <= 9 && !numero.startsWith('39')) {
        numero = '39' + numero;
    }
    ...
    // 🔥 LA MAGIA: DECIDE QUALE MESSAGGIO USARE IN BASE AI 10 EURO! 🔥
    let templateScelto = "";
    if (clienteManualeScelto.bonus < 10) {
        templateScelto = localStorage.getItem('impostazioni_msg_saldo_min') || MSG_SALDO_MINORE_DEFAULT;
    } else {
        templateScelto = localStorage.getItem('impostazioni_msg_saldo_max') || MSG_SALDO_MAGGIORE_DEFAULT;
    }

    let messaggio = templateScelto
        .replace(/{NOME}/g, clienteManualeScelto.nome.toUpperCase())
        .replace(/{SCHEDA}/g, clienteManualeScelto.scheda)
        .replace(/{SALDO_INIZIALE}/g, "-")
        .replace(/{PUNTI_CARICATI}/g, "-")
        .replace(/{PUNTI_SCARICATI}/g, "-")
...
window.confermaEliminazioneMagazzino = function () {
    let inputCodice = document.getElementById('mag-codice');
    // Prende il codice originale in memoria. Se non c'è, prende quello scritto nel campo.
    idDaEliminare = inputCodice.dataset.codiceOriginale || inputCodice.value.trim();
    ...
    arrayDati.forEach((item) => {
        ...
        if (item.sorgente === 'vendita') {
            ...
            if (item.articoli && item.articoli.length > 0) {
                ...
                div.style.borderLeft = '4px solid transparent'; // 🔥 FIX: Ingombro fisso per evitare lo scivolamento

                div.onmouseover = function () {
                    this.style.backgroundColor = "rgba(191, 149, 63, 0.25)"; // Sfondo oro elegante
                    this.style.color = "#fcf6ba"; // Testo oro brillante per massimo contrasto
                    this.style.borderLeft = "4px solid #bf953f"; // Bordo laterale dorato
                };
...
window.apriDettaglioScontrino = function (venditaRaw) {
    ...
    if (venditaRaw.ARTICOLI && venditaRaw.ARTICOLI.length > 0) {
        venditaRaw.ARTICOLI.forEach(art => {
            // --- NUOVA LOGICA: Gestione visiva Valore o Punti ---
            let valoreVisualizzato = `€ ${art.IMPORTO.toLocaleString('it-IT', { minimumFractionDigits: 2 })}`;

            // Se l'articolo è un movimento manuale punti, mostra i punti al posto degli Euro
            if (art.CODICE === "PUNTI") {
                if (venditaRaw.PUNTI_CARICATI > 0) {
                    valoreVisualizzato = `<span style="color: #00cc66; font-weight: bold;">+ ${venditaRaw.PUNTI_CARICATI}</span>`;
                } else if (venditaRaw.PUNTI_SCARICATI > 0) {
                    valoreVisualizzato = `<span style="color: #ff4d4d; font-weight: bold;">- ${venditaRaw.PUNTI_SCARICATI}</span>`;
                } else {
                    valoreVisualizzato = `0`;
                }
            }
    ...
    if (venditaRaw.CLIENTE !== "Nessuno") {
        ...
        document.getElementById('det-punti-guadagnati').textContent = `Punti Guadagnati: ${venditaRaw.PUNTI_CARICATI > 0 ? '+' : ''}${venditaRaw.PUNTI_CARICATI || 0}`;
...
function magDisegnaLista(arrayProdotti) {
    ...
    arrayProdotti.forEach(p => {
        ...
        // 🔥 LOGICA DINAMICA: Impostazioni di default
        let iconaGiacenza = p.is_virtuale ? '♾️' : '📦'; // 🪄 Indica che la giacenza è virtuale (BOM)
        let unitaMisura = p.is_virtuale ? 'max' : 'pz';

        // 🔥 LOGICA DINAMICA: Controlla se l'articolo è un Fusto Madre
        if (p.descrizione.toUpperCase().includes("FUSTO MADRE")) {
            iconaGiacenza = "💧";
            unitaMisura = "ml";
        }
        ...
        div.innerHTML = `
            ...
            <div class="crm-list-dati">
                ...
                <span style="color: ${giacenzaColore}; font-weight: bold;">${iconaGiacenza} ${p.giacenza} ${unitaMisura}</span>
...
function magCaricaScheda(p) {
    ...
    let inputCodice = document.getElementById('mag-codice');
    inputCodice.value = p.codice;
    inputCodice.disabled = false; // Il campo ora rimane modificabile
    inputCodice.dataset.codiceOriginale = p.codice; // Salva la memoria del vecchio codice
...
window.magNuovoProdotto = function () {
    ...
    let inputCodice = document.getElementById('mag-codice');
    inputCodice.value = '';
    inputCodice.disabled = false;
    inputCodice.dataset.codiceOriginale = ''; // Svuota la memoria del codice
...
window.magSalvaProdotto = async function () {
    let codiceInput = document.getElementById('mag-codice');
    let codiceNuovo = codiceInput.value.trim();
    let codiceOriginale = codiceInput.dataset.codiceOriginale || "";
    ...
    if (codiceNuovo === '' || descrizione === '' || prezzoVen < 0) {
        mostraAvvisoModale("Compila i campi obbligatori:<br>- Codice<br>- Descrizione<br>(Il prezzo di vendita non può essere negativo)");
        return;
    }

    // 1. CONTROLLO DUPLICATI: Se è un nuovo inserimento o il codice è stato modificato
    if (codiceOriginale === "" || codiceOriginale !== codiceNuovo) {
        let prodottoEsistente = await getRecordById('magazzino', codiceNuovo);
        if (prodottoEsistente) {
            mostraAvvisoModale(`⚠️ <b>ATTENZIONE</b><br><br>Il codice/barcode <b>${codiceNuovo}</b> è già utilizzato dall'articolo:<br><i>${prodottoEsistente.descrizione}</i><br><br>Scegli un codice diverso.`);
            return;
        }
    }
    ...
    let nuovoProdotto = {
        ...
        tipo: "PZ",
        timestamp_sync: Date.now() // 🔥 TIMBRO ANTI-GHOST FONDAMENTALE
    };

    let tx = db.transaction('magazzino', 'readwrite');
    let store = tx.objectStore('magazzino');

    // 2. ELIMINAZIONE VECCHIA SCHEDA (necessario se il codice è stato cambiato)
    if (codiceOriginale !== "" && codiceOriginale !== codiceNuovo) {
        store.delete(codiceOriginale);

        // Pulisce anche Firebase per evitare prodotti orfani
        if (navigator.onLine && FIREBASE_URL !== "") {
            fetch(`${FIREBASE_URL}/magazzino/${codiceOriginale}.json`, { method: 'DELETE' }).catch(e => console.log(e));
        }
    }

    // 3. INSERIMENTO SCHEDA AGGIORNATA
    store.put(nuovoProdotto);

    tx.oncomplete = async () => {
        if (typeof salvaProdottoCloud === "function") salvaProdottoCloud(nuovoProdotto);

        // Aggiorna la memoria in modo che l'operatore possa continuare a modificare e salvare
        codiceInput.dataset.codiceOriginale = codiceNuovo;
...
// 2.5 Ripristino Backup Totale da file JSON (Con Sincronizzazione Cloud Bidirezionale)
window.importaBackupJSON = function (event) {
    ...
    reader.onload = async function (e) {
        try {
            ...
            let categorieDaAggiungere = new Set(); // Raccoglitore categorie
            ...
            tx.oncomplete = async () => {
                ...
                // 🔥 SINCRONIZZAZIONE BIDIREZIONALE: Spinge i dati appena importati su Firebase
                if (navigator.onLine && FIREBASE_URL !== "") {
                    mostraAvvisoModale("⏳ Backup caricato in locale.<br>Sincronizzazione col Cloud in corso...<br><span style='font-size:1.4vh; color:#b3d9ff;'>(Potrebbe richiedere qualche istante in base alla grandezza dello storico)</span>");

                    try {
                        // 1. Spinge i Clienti
                        if (backup.clienti) {
                            for (let c of backup.clienti) {
                                if (typeof salvaClienteCloud === "function") await salvaClienteCloud(c);
                            }
                        }

                        // 2. Spinge il Magazzino
                        if (backup.magazzino) {
                            for (let p of backup.magazzino) {
                                if (typeof salvaProdottoCloud === "function") await salvaProdottoCloud(p);
                            }
                        }

                        // 3. Spinge le Gift Card
                        if (backup.giftcards) {
                            for (let gc of backup.giftcards) {
                                if (typeof salvaGiftCardCloud === "function") await salvaGiftCardCloud(gc);
                            }
                        }

                        // 4. Spinge i Vouchers
                        if (backup.vouchers) {
                            for (let v of backup.vouchers) {
                                if (typeof salvaVoucherCloud === "function") await salvaVoucherCloud(v);
                            }
                        }

                        // 5. Spinge lo Storico Vendite e le Vendite Live (Cruscotto)
                        if (backup.vendite) {
                            for (let v of backup.vendite) {
                                if (typeof salvaVenditaCloud === "function") await salvaVenditaCloud(v);
                                if (typeof inviaVenditaLive === "function") await inviaVenditaLive(v);
                            }
                        }

                        // 6. Spinge lo Storico Movimenti e i Movimenti Live (Cruscotto)
                        if (backup.movimenti_cassa) {
                            for (let m of backup.movimenti_cassa) {
                                if (typeof salvaMovimentoCloud === "function") await salvaMovimentoCloud(m);
                                if (typeof inviaMovimentoLive === "function") await inviaMovimentoLive(m);
                            }
                        }

                        // 7. Spinge le Chiusure Fiscali (Z)
                        if (backup.chiusure) {
                            for (let ch of backup.chiusure) {
                                if (typeof salvaChiusuraCloud === "function") await salvaChiusuraCloud(ch);
                            }
                        }

                        // 8. Spinge gli Ordini ai Fornitori
                        if (backup.ordini) {
                            for (let o of backup.ordini) {
                                if (typeof salvaOrdineCloud === "function") await salvaOrdineCloud(o);
                            }
                        }

                        mostraAvvisoModale("✅ <b>Backup ripristinato e Sincronizzato col Cloud!</b><br><br>Tutti i dati, le categorie, gli archivi storici e i cruscotti live sono stati ricaricati nel sistema e su Firebase.");
                    } catch (e) {
                        console.error("Errore durante la sincronizzazione post-ripristino", e);
                        mostraAvvisoModale("⚠️ <b>Backup ripristinato in locale.</b><br>Si è verificato un errore durante l'invio massivo al Cloud. Attendi il prossimo ciclo dell'Autopilota per il completamento.");
                    }
                } else {
                    mostraAvvisoModale("✅ <b>Backup ripristinato in locale!</b><br><br>La sincronizzazione col Cloud avverrà non appena la connessione sarà disponibile.");
                }
                event.target.value = '';
            };
        } catch (error) {
...
// ==========================================
// 💬 TESTI DI DEFAULT WHATSAPP
// ==========================================
// 4.1. Salvataggio Impostazioni Personalizzate
const MSG_VENDITA_MINORE_DEFAULT = "CHEMARIA FIDELITY\n\nCiao {NOME} 👋\nGrazie per il tuo acquisto! 🛍️\n\n💳 Card N°: {SCHEDA}\n\n━━━━━━━━━━━━━━━\n➕ Punti Caricati: {PUNTI_CARICATI}\n➖ Punti Scaricati: {PUNTI_SCARICATI}\n⭐ Nuovo Saldo: {PUNTI}\n💰 Bonus DISPONIBILE: € {BONUS}\n━━━━━━━━━━━━━━━\n\nTi aspettiamo presto in negozio per raggiungere il tuo premio! 😉\n\n📅 {DATA}\n🕒 {ORA}\n\n🙏 Grazie per averci scelto";

const MSG_VENDITA_MAGGIORE_DEFAULT = "CHEMARIA FIDELITY\n\nCiao {NOME} 👋\nGrazie per il tuo acquisto! 🛍️\nAbbiamo una buona notizia per te! 🎉\n\n💳 Card N°: {SCHEDA}\n\n━━━━━━━━━━━━━━━\n➕ Punti Caricati: {PUNTI_CARICATI}\n➖ Punti Scaricati: {PUNTI_SCARICATI}\n⭐ Nuovo Saldo: {PUNTI}\n💰 Bonus DISPONIBILE: € {BONUS}\n━━━━━━━━━━━━━━━\n\n👉 Usa subito il tuo bonus e risparmia sui tuoi prossimi acquisti!\n⏳ Non lasciarlo inutilizzato! 😉\n\n📅 {DATA}\n🕒 {ORA}\n\n🙏 Grazie per averci scelto";

const MSG_SALDO_MINORE_DEFAULT = "CHEMARIA FIDELITY\n\n👋 {NOME}\n💳 {SCHEDA}\n\nEcco il promemoria del tuo saldo punti aggiornato:\n\n━━━━━━━━━━━━━━━\n⭐ {PUNTI}\n💰 € {BONUS}\n━━━━━━━━━━━━━━━\n\nTi aspettiamo in negozio per riscattare il tuo premio!\n\n📅 {DATA}\n🕒 {ORA}\n\n🙏 Grazie per averci scelto";

const MSG_SALDO_MAGGIORE_DEFAULT = "CHEMARIA FIDELITY\n\n👋 {NOME}\n💳 {SCHEDA}\n\nAbbiamo una buona notizia per te! 🎉\n\n━━━━━━━━━━━━━━━\n⭐ {PUNTI}\n💰 € {BONUS}\n━━━━━━━━━━━━━━━\n\n👉 Usa subito il tuo bonus e risparmia sui tuoi prossimi acquisti!\n\n⏳ Non lasciarlo inutilizzato! Ti aspettiamo in negozio 😉\n\n📅 {DATA}\n🕒 {ORA}\n\n🙏 Grazie per averci scelto";

// ==========================================
// ⚙️ LOGICA IMPOSTAZIONI GENERALI
// ==========================================
window.caricaImpostazioniAvanzate = function () {
    // Carica PIN
    let pinAttivo = localStorage.getItem('impostazioni_pin_attivo');
    let togglePin = document.getElementById('impostazioni-pin-toggle');
    if (togglePin) togglePin.checked = (pinAttivo !== 'false');

    // Carica Dati Scontrino
    let impNome = document.getElementById('imp-stampa-nome');
    if (impNome) impNome.value = localStorage.getItem('imp_stampa_nome') || "";
    
    let impIndirizzo = document.getElementById('imp-stampa-indirizzo');
    if (impIndirizzo) impIndirizzo.value = localStorage.getItem('imp_stampa_indirizzo') || "";
    
    let impPiva = document.getElementById('imp-stampa-piva');
    if (impPiva) impPiva.value = localStorage.getItem('imp_stampa_piva') || "";
    
    let impFooter = document.getElementById('imp-stampa-footer');
    if (impFooter) impFooter.value = localStorage.getItem('imp_stampa_footer') || "Grazie e Arrivederci!";
    ...
    // Carica Dati SumUp
    let sumupId = document.getElementById('impostazioni-sumup-id');
    if (sumupId) sumupId.value = localStorage.getItem('sumup_client_id') || "";
    
    let sumupSecret = document.getElementById('impostazioni-sumup-secret');
    if (sumupSecret) sumupSecret.value = localStorage.getItem('sumup_client_secret') || "";
};
...
window.salvaImpostazioniAvanzate = function () {
    ...
    let togglePin = document.getElementById('impostazioni-pin-toggle');
    if (togglePin) {
        localStorage.setItem('impostazioni_pin_attivo', togglePin.checked ? 'true' : 'false');
    }

    // Salva Dati Scontrino
    let impNome = document.getElementById('imp-stampa-nome');
    if (impNome) localStorage.setItem('imp_stampa_nome', impNome.value.trim());

    let impIndirizzo = document.getElementById('imp-stampa-indirizzo');
    if (impIndirizzo) localStorage.setItem('imp_stampa_indirizzo', impIndirizzo.value.trim());

    let impPiva = document.getElementById('imp-stampa-piva');
    if (impPiva) localStorage.setItem('imp_stampa_piva', impPiva.value.trim());

    let impFooter = document.getElementById('imp-stampa-footer');
    if (impFooter) localStorage.setItem('imp_stampa_footer', impFooter.value.trim());
    ...
    // Salva Dati SumUp
    let sumupId = document.getElementById('impostazioni-sumup-id');
    if (sumupId) localStorage.setItem('sumup_client_id', sumupId.value.trim());

    let sumupSecret = document.getElementById('impostazioni-sumup-secret');
    if (sumupSecret) localStorage.setItem('sumup_client_secret', sumupSecret.value.trim());
...
// ==========================================
let tipoEditorAttivo = 'vendita_min';

window.apriEditorMessaggio = function (tipo) {
    tipoEditorAttivo = tipo;
    let testoAttuale = "";
    let titolo = "";
    
    if (tipo === 'vendita_min') {
        testoAttuale = document.getElementById('impostazioni-msg-vendita-min').value;
        titolo = "✏️ SCONTRINO (< 10€)";
    } else if (tipo === 'vendita_max') {
        testoAttuale = document.getElementById('impostazioni-msg-vendita-max').value;
        titolo = "✏️ SCONTRINO (≥ 10€)";
    } else if (tipo === 'saldo_min') {
        testoAttuale = document.getElementById('impostazioni-msg-saldo-min').value;
        titolo = "✏️ SALDO (< 10€)";
    } else if (tipo === 'saldo_max') {
        testoAttuale = document.getElementById('impostazioni-msg-saldo-max').value;
        titolo = "✏️ SALDO (≥ 10€)";
    }

    let etichettaTitolo = document.getElementById('titolo-editor-msg');
    if (etichettaTitolo) etichettaTitolo.innerHTML = titolo;

    let editor = document.getElementById('editor-messaggio-textarea');
    if (editor) editor.value = testoAttuale;
...
window.confermaEditorMessaggio = function () {
    ...
    if (tipoEditorAttivo === 'vendita_min') {
        document.getElementById('impostazioni-msg-vendita-min').value = nuovoTesto;
    } else if (tipoEditorAttivo === 'vendita_max') {
        document.getElementById('impostazioni-msg-vendita-max').value = nuovoTesto;
    } else if (tipoEditorAttivo === 'saldo_min') {
        document.getElementById('impostazioni-msg-saldo-min').value = nuovoTesto;
    } else if (tipoEditorAttivo === 'saldo_max') {
        document.getElementById('impostazioni-msg-saldo-max').value = nuovoTesto;
    }
...
window.apriImpostazioniWhatsApp = function () {
    document.getElementById('impostazioni-msg-vendita-min').value = localStorage.getItem('impostazioni_msg_vendita_min') || MSG_VENDITA_MINORE_DEFAULT;
    document.getElementById('impostazioni-msg-vendita-max').value = localStorage.getItem('impostazioni_msg_vendita_max') || MSG_VENDITA_MAGGIORE_DEFAULT;
    document.getElementById('impostazioni-msg-saldo-min').value = localStorage.getItem('impostazioni_msg_saldo_min') || MSG_SALDO_MINORE_DEFAULT;
    document.getElementById('impostazioni-msg-saldo-max').value = localStorage.getItem('impostazioni_msg_saldo_max') || MSG_SALDO_MAGGIORE_DEFAULT;
...
window.salvaImpostazioniWhatsApp = function () {
    localStorage.setItem('impostazioni_msg_vendita_min', document.getElementById('impostazioni-msg-vendita-min').value);
    localStorage.setItem('impostazioni_msg_vendita_max', document.getElementById('impostazioni-msg-vendita-max').value);
    localStorage.setItem('impostazioni_msg_saldo_min', document.getElementById('impostazioni-msg-saldo-min').value);
    localStorage.setItem('impostazioni_msg_saldo_max', document.getElementById('impostazioni-msg-saldo-max').value);

    mostraAvvisoModale("Tutti i Template WhatsApp aggiornati con successo!");
    chiudiModale('modal-impostazioni-whatsapp');
    apriModale('modal-impostazioni-menu');
};
...
window.scaricaMagazzinoDalCloud = async function () {
    ...
    try {
        ...
        // 🔥 NUOVA LOGICA BIDIREZIONALE: PC -> CLOUD (Sincronizzazione del lavoro Offline)
        let magazzinoLocaleCompleto = await getAll('magazzino');

        magazzinoLocaleCompleto.forEach(prodLocale => {
            // Cerca se questo prodotto esiste sul Cloud appena scaricato
            let prodCloud = magazzinoCloud ? magazzinoCloud[prodLocale.codice] : null;

            // 1. Creato Offline: Esiste sul PC ma non esiste ancora su Firebase
            let mancaSulCloud = !prodCloud;

            // 2. Modificato Offline: Il PC ha un timbro orario più fresco rispetto a Firebase
            let localePiuRecente = prodCloud && prodLocale.timestamp_sync && (!prodCloud.timestamp_sync || prodLocale.timestamp_sync > prodCloud.timestamp_sync);

            if (mancaSulCloud || localePiuRecente) {
                console.log(`☁️ Sync UP Automatico (Post-Offline): Aggiornamento cloud per [${prodLocale.codice}]`);
                // Riversa il prodotto aggiornato su Firebase
                if (typeof salvaProdottoCloud === "function") {
                    salvaProdottoCloud(prodLocale);
                }
            }
        });

        // Eventuale aggiornamento dell'interfaccia grafica
        if (aggiornamentiFatti > 0 && typeof magCaricaLista === "function") {
            magCaricaLista();
        }

    } catch (e) {
        console.error("Errore durante la sincronizzazione bidirezionale:", e);
    }
};
...
window.salvaVenditaCloud = async function (vendita) {
    ...
    try {
        ...
        console.error("Errore sincronizzazione vendita cloud:", e);
    }
};
...
window.caricaListaOrdini = function () {
    ...
    req.onsuccess = function () {
        ...
        filtrati.forEach(o => {
            ...
            // --- FIX: CALCOLO TOTALE CON LOGICA FUSTO ---
            let totale = o.articoli.reduce((acc, a) => {
                let costoBase = a.prezzoAcquisto || 0;
                let isFusto = (a.descrizione && a.descrizione.includes("FUSTO MADRE"));
                let costoRealeUnitario = isFusto ? (costoBase * 250) : costoBase;
                return acc + (costoRealeUnitario * a.qta_ordinata);
            }, 0);
...
window.generaOrdineDaSottoscorta = function () {
    ...
    req.onsuccess = function () {
        ...
        daOrdinare.forEach(p => {
            ...
            // --- FIX: CONVERSIONE AUTOMATICA FUSTI (Pezzi da 250ml) ---
            let isFusto = (p.descrizione && p.descrizione.includes("FUSTO MADRE"));
            if (isFusto) {
                // I millilitri mancanti vengono divisi per 250. 
                // Math.ceil arrotonda sempre per eccesso (es. se mancano 300ml, ordina 2 fusti interi)
                qtaDaComprare = Math.ceil(qtaDaComprare / 250);
            }
        ...
        for (let fornitore in raggruppati) {
            ...
            if (typeof salvaOrdineCloud === "function") salvaOrdineCloud(nuovoOrdine);
            contatoreBozze++;
        }
...
window.disegnaArticoliOrdine = function () {
    let tbody = document.getElementById('body-articoli-ordine');
    if (!tbody) return;
    tbody.innerHTML = '';
    let totaleOrdine = 0;

    if (!ordineApertoAttuale || !ordineApertoAttuale.articoli) return;

    let isModificabile = (ordineApertoAttuale.stato === 'BOZZA' || ordineApertoAttuale.stato === 'INVIATO');

    ordineApertoAttuale.articoli.forEach((art, idx) => {
        let isFusto = (art.descrizione && art.descrizione.includes("FUSTO MADRE"));
        let costoBaseDb = art.prezzoAcquisto || 0;

        // --- NUOVA LOGICA: CALCOLO PREZZO FUSTI ---
        // Se è un fusto, il prezzo memorizzato è per singolo millilitro.
        // Dobbiamo calcolare il costo di un fusto intero (250ml) per la visualizzazione e il totale.
        let costoUnitario = isFusto ? (costoBaseDb * 250) : costoBaseDb;
        let totRiga = costoUnitario * art.qta_ordinata;

        totaleOrdine += totRiga;

        let labelExtra = "";
        if (isFusto) {
            let mlTotali = art.qta_ordinata * 250;
            if (mlTotali >= 1000) {
                let litri = mlTotali / 1000;
                labelExtra = `<div style="font-size: 11px; color: #00cc66; margin-top: 5px; font-weight: bold;">(Tot: ${litri.toLocaleString('it-IT')} Litri)</div>`;
            } else {
                labelExtra = `<div style="font-size: 11px; color: #00cc66; margin-top: 5px; font-weight: bold;">(Tot: ${mlTotali} ml)</div>`;
            }
        }

        // Casella quantità
        let inputQta = isModificabile
            ? `<input type="number" value="${art.qta_ordinata}" min="1" style="width: 80px; text-align: center; border-radius: 4px; border: 1px solid #4d88ff; background: rgba(0,0,0,0.5); color: white; padding: 5px;" onchange="aggiornaQtaArticoloOrdine(${idx}, this.value)">${labelExtra}`
            : `<b style="font-size: 16px;">${art.qta_ordinata}</b>${labelExtra}`;

        // Pulsante di rimozione
        let btnRimuovi = isModificabile
            ? `<button onclick="rimuoviArticoloOrdine(${idx})" style="background: rgba(255, 51, 51, 0.15); border: 1px solid #ff3333; color: #ffcccc; border-radius: 4px; padding: 6px 10px; cursor: pointer; transition: 0.2s; font-size: 12px;" onmouseover="this.style.background='rgba(255, 51, 51, 0.4)'" onmouseout="this.style.background='rgba(255, 51, 51, 0.15)'" title="Rimuovi Articolo">❌</button>`
            : ``;
        ...
        tr.innerHTML = `
            ...
            <td style="padding: 10px; text-align: center;">€ ${costoUnitario.toLocaleString('it-IT', { minimumFractionDigits: 2 })}</td>
            <td style="padding: 10px; text-align: center; vertical-align: middle;">${inputQta}</td>
            ...
            <td style="padding: 10px; text-align: center; vertical-align: middle;">${btnRimuovi}</td>
        `;
        tbody.appendChild(tr);
    });

    let divTotale = document.getElementById('ord-totale');
    if (divTotale) {
        divTotale.textContent = `€ ${totaleOrdine.toLocaleString('it-IT', { minimumFractionDigits: 2 })}`;
    }
}
...
function disegnaTabellaCarico() {
    ...
    ordineApertoAttuale.articoli.forEach((art, idx) => {
        ...
        let tr = document.createElement('tr');
        // 🔥 FIX: Imposta il colore di partenza se l'articolo era già stato spuntato interamente
        tr.style.backgroundColor = (art.qta_ricevuta === art.qta_ordinata && art.qta_ordinata > 0) ? 'rgba(0, 204, 102, 0.2)' : 'transparent';
        tr.style.transition = 'background-color 0.2s'; // Effetto fluido per il cambio colore
        ...
        tr.innerHTML = `
            ...
            <td style="padding: 10px; text-align: center;">
                <input type="number" id="qta-carico-${idx}" value="${art.qta_ricevuta}" min="0" max="${art.qta_ordinata}" style="width: 80px; height: 35px; text-align: center; font-size: 2.2vh; font-weight: bold; color: #00cc66; background: rgba(0,0,0,0.5); border: 1px solid #00cc66; box-sizing: border-box; border-radius: 4px; outline: none;" oninput="impostaQtaCaricoManuale(${idx}, this.value)">
            </td>
        `;
        tbody.appendChild(tr);
    });
...
window.impostaQtaCaricoManuale = function (idx, nuovaQtaStr) {
    let art = ordineApertoAttuale.articoli[idx];
    ...
    if (isNaN(nuovaQta) || nuovaQta < 0) {
        nuovaQta = 0;
    } else if (nuovaQta > art.qta_ordinata) {
        nuovaQta = art.qta_ordinata;
    }

    // Aggiorna il database in memoria
    art.qta_ricevuta = nuovaQta;

    // Selezioniamo il campo in cui stiamo scrivendo
    let inputCampo = document.getElementById(`qta-carico-${idx}`);
    if (inputCampo) {
        if (parseInt(inputCampo.value) !== nuovaQta) {
            inputCampo.value = nuovaQta;
        }

        // Troviamo la riga e aggiorniamo la grafica "al volo"
        let riga = inputCampo.closest('tr');
        if (riga) {
            let cellaDescrizione = riga.querySelectorAll('td')[1];
            if (cellaDescrizione) {
                cellaDescrizione.style.fontWeight = nuovaQta > 0 ? 'bold' : 'normal';
            }

            // 🔥 FIX: Ricolora la riga quando la spunta è completa (quantità arrivata = quantità ordinata)
            if (nuovaQta === art.qta_ordinata && art.qta_ordinata > 0) {
                riga.style.backgroundColor = 'rgba(0, 204, 102, 0.2)'; // Verde semitrasparente per riga completata
            } else {
                riga.style.backgroundColor = 'transparent'; // Torna trasparente se riduci il numero
            }
        }
    }
};
...
// Conferma il carico (Versione Definitiva con Storico Intatto e Moltiplicatore Fusti)
window.confermaCaricoMerce = function () {
        ...
        if (quantitaDaAggiungere > 0) {
            ...
            req.onsuccess = function () {
                if (req.result) {
                    ...
                    // --- NUOVA LOGICA: MOLTIPLICATORE FUSTI (1 PEZZO = 250 ML) ---
                    let qtaDaCaricare = quantitaDaAggiungere;
                    let isFusto = (art.descrizione && art.descrizione.includes("FUSTO MADRE"));

                    if (isFusto) {
                        qtaDaCaricare = quantitaDaAggiungere * 250; // Moltiplica per i ml del fusto
                    }

                    // Carica la giacenza effettiva
                    prodotto.giacenza = parseInt(prodotto.giacenza || 0) + qtaDaCaricare;

                    if (!prodotto.lotti) prodotto.lotti = [];
                    prodotto.lotti.push({
                        codice_lotto: document.getElementById('carico-input-lotto').value.trim() || `L-${Date.now()}`,
                        qta_iniziale: qtaDaCaricare, // Anche il lotto ragiona in millilitri
                        qta_residua: qtaDaCaricare,
                    ...
                    // Calcolo costo unitario (attenziona che il costo del fusto è per pezzo, non per ml)
                    // Nelle statistiche o nel magazzino il costo di acquisto si riferirà al Fusto Intero.
                    let nuovoCosto = (parseFloat(art.prezzoAcquisto) || 0) + quotaSpesePerPezzo;
                    prodotto.prezzoAcquisto = parseFloat(nuovoCosto.toFixed(2));

                    prodotto.timestamp_sync = Date.now(); // 🔥 TIMBRO ANTI-GHOST
                    storeMag.put(prodotto);
                    if (typeof salvaProdottoCloud === "function") salvaProdottoCloud(prodotto); // 🔥 AGGIORNA IL CLOUD

                    // La stampa etichette stampa per pezzi fisici, non per ml!
                    if (stampaEtichette) {
                        aggiungiACodaStampa({
                        ...
                        }, quantitaDaAggiungere); // Qui usiamo "quantitaDaAggiungere" (i fusti fisici)
                    }
                }
            };
        }
    ...
    tx.oncomplete = function () {
        ...
        if (stampaEtichette) {
            ...
            apriModale('modal-stampa-etichette');
            mostraAvvisoModale(`✅ <b>CARICO COMPLETATO</b><br><br>La merce è stata caricata (i fusti sono stati convertiti in ml).<br>Le etichette fisiche le trovi nella coda di stampa.`);
        } else {
            mostraAvvisoModale(`✅ <b>CARICO COMPLETATO</b><br><br>La merce è stata caricata in magazzino.`);
        }
    };
};
...
window.filtraDashboardGiftCard = async function () {
    // 1. Estrae ENTRAMBI gli archivi
    let vouchers = await getAll('vouchers');
    let giftcards = await getAll('giftcards');

    // 2. Li fonde in un unico super-array
    let tuttiIDocumenti = [...vouchers, ...giftcards];

    // Recupera i valori dai filtri
    let statoScelto = document.getElementById('filtro-stato-gc') ? document.getElementById('filtro-stato-gc').value : "TUTTI";
    let tipoScelto = document.getElementById('filtro-tipo-gc') ? document.getElementById('filtro-tipo-gc').value : "TUTTI";
    let testoRicerca = document.getElementById('filtro-ricerca-gc') ? document.getElementById('filtro-ricerca-gc').value.toLowerCase().trim() : "";
    ...
    // Ordinamento Intelligente: dal più recente al più vecchio
    tuttiIDocumenti.sort((a, b) => {
        let dataA = a.dataEmissione || a.data_emissione || "01/01/1970";
        let dataB = b.dataEmissione || b.data_emissione || "01/01/1970";

        // Converte le date europee (DD/MM/YYYY) o ISO in un formato calcolabile
        let parsedA = dataA.includes('/') ? new Date(dataA.split('/').reverse().join('-')) : new Date(dataA);
        let parsedB = dataB.includes('/') ? new Date(dataB.split('/').reverse().join('-')) : new Date(dataB);
        return parsedB - parsedA;
    });

    tuttiIDocumenti.forEach(v => {
        // --- NORMALIZZAZIONE DATI (Unifica i campi di Voucher e GiftCard) ---
        let isReso = !v.codice.startsWith('GC'); // Se NON inizia per GC, è un reso!
        let saldoReale = isReso ? Math.abs(v.importo || v.valore_iniziale || 0) : (v.saldo || 0);
        let valoreIniziale = isReso ? Math.abs(v.importo || v.valore_iniziale || 0) : (v.importoIniziale || 0);
        let dataEmiss = v.dataEmissione || v.data_emissione || "N/D";

        // --- CALCOLO STATO ---
        let isAttiva = false;
        if (isReso) {
            // I buoni reso vengono eliminati dal DB appena usati, quindi se sono qui sono attivi
            isAttiva = true;
        } else {
            isAttiva = (v.stato === 'ATTIVA' && v.saldo > 0);
        }

        let statoTesto = isAttiva ? "ATTIVA" : "ESAURITA";
        let statoColore = isAttiva ? "#00cc66" : "#888";
        let iconaTipo = isReso ? "🔄 Buono Reso" : "🎁 Gift Card";
        let testoScadenza = isReso ? "12 Mesi" : (v.scadenza || "Senza Scadenza");
        ...
        if (testoRicerca !== "") {
            let stringaRicerca = `${v.codice} ${valoreIniziale} ${saldoReale}`.toLowerCase();
            if (!stringaRicerca.includes(testoRicerca)) return;
        }

        // Calcolo per i quadrati riepilogativi in alto
        if (isAttiva) {
            totaliAttive++;
            debitoTotale += parseFloat(saldoReale);
        }
        ...
        tr.innerHTML = `
            <td style="padding: 15px 10px; text-align: center; font-family: monospace; font-size: 14px; color: #ffcc00; font-weight: bold;">
                ${v.codice}<br>
                <span style="font-family: sans-serif; font-size: 12px; color: #b3d9ff;">${iconaTipo}</span>
            </td>
            <td style="padding: 15px 10px; text-align: center; font-size: 13px; color: #cccccc;">${dataEmiss}</td>
            <td style="padding: 15px 10px; text-align: center; font-size: 13px; color: #cccccc;">${testoScadenza}</td>
            <td style="padding: 15px 10px; text-align: center; font-size: 13px; color: #aaaaaa;">€ ${valoreIniziale.toLocaleString('it-IT', { minimumFractionDigits: 2 })}</td>
            <td style="padding: 15px 10px; text-align: center; color: ${statoColore}; font-weight: bold; font-size: 13px;">€ ${saldoReale.toLocaleString('it-IT', { minimumFractionDigits: 2 })}</td>
            <td style="padding: 15px 10px; text-align: center; color: ${statoColore}; font-weight: bold; font-size: 13px;">${statoTesto}</td>
            <td style="padding: 15px 10px; text-align: center;">
                <button class="btn-modal" style="background: rgba(191, 149, 63, 0.15); border: 1px solid #bf953f; color: #fcf6ba; padding: 8px 15px; margin: 0; font-size: 10px; font-weight: bold; letter-spacing: 1px; cursor: pointer; transition: 0.3s; border-radius: 4px; text-shadow: 0 0 5px rgba(252, 246, 186, 0.5); box-shadow: inset 0 0 10px rgba(191, 149, 63, 0.1);" onmouseover="this.style.background='rgba(191, 149, 63, 0.4)'; this.style.boxShadow='0 0 15px rgba(191, 149, 63, 0.5)';" onmouseout="this.style.background='rgba(191, 149, 63, 0.15)'; this.style.boxShadow='inset 0 0 10px rgba(191, 149, 63, 0.1)';" onclick="verificaRapidaGiftCard('${v.codice}')">🔍 DETTAGLIO</button>
            </td>
        `;
        tbody.appendChild(tr);
    });

    // Aggiorna le statistiche totali in cima alla pagina
    let spanAttive = document.getElementById('dash-gc-attive');
    let spanDebito = document.getElementById('dash-gc-debito');
    if (spanAttive) spanAttive.innerText = totaliAttive;
    if (spanDebito) spanDebito.innerText = '€ ' + debitoTotale.toLocaleString('it-IT', { minimumFractionDigits: 2 });
};
...
window.verificaRapidaGiftCard = async function (codiceInput) {
    ...
    // 1. Cerca prima nell'archivio delle Gift Card
    let gc = await getRecordById('giftcards', codice);
    let isReso = false;

    // 2. Se non lo trova, prova a cercarlo nell'archivio dei Buoni Reso
    if (!gc) {
        let vou = await getRecordById('vouchers', codice);
        if (vou) {
            gc = {
                codice: vou.codice,
                saldo: parseFloat(vou.importo || vou.valore_iniziale || 0),
                dataEmissione: vou.dataEmissione || vou.data_emissione || "N/D",
    ...
    if (gc) {
        ...
        if (isReso) {
            if (gc.saldo <= 0) { statoLabel.textContent = "ESAURITO"; statoLabel.style.color = "#888"; }
            else { statoLabel.textContent = "ATTIVO (BUONO RESO)"; statoLabel.style.color = "#00cc66"; }
        } else {
        ...
        if (!gc.storico || gc.storico.length === 0) {
            htmlStorico = `<div style='color:#888; text-align:center;'>${isReso ? 'Buono monouso.' : 'Nessun movimento registrato.'}</div>`;
        } else {
            let storicoInverso = [...gc.storico].reverse();
            storicoInverso.forEach(st => {
                let segno = st.importo < 0 ? "" : "+";
        ...
        let inputRicarica = document.getElementById('input-ricarica-gc');
        if (inputRicarica) {
            inputRicarica.value = '';
            inputRicarica.disabled = isReso;
            inputRicarica.placeholder = isReso ? "Non ricaricabile" : "Importo €";
        }

        // 🔥 NUOVO: Cambio dinamico del Titolo e del Colore
        let titoloModale = document.querySelector('#modal-dettaglio-gc .modal-titolo');
        if (titoloModale) {
            titoloModale.textContent = isReso ? "🔎 DETTAGLIO RESO" : "🔎 DETTAGLIO GIFT CARD";
            titoloModale.style.color = isReso ? "#b3d9ff" : "#fcf6ba"; // Azzurro per resi, Oro per Gift Card
        }

        apriModale('modal-dettaglio-gc');
    } else {
        mostraAvvisoModale(`Nessuna Gift Card o Buono Reso trovato con il codice <b>${codice}</b>`);
        document.getElementById('input-verifica-gc').value = '';
    }
};
...
window.apriModaleScegliEssenza = async function (formato) {
    ...
    // --- NUOVO FILTRO: Carica solo le fragranze liquide CON GIACENZA MAGGIORE DI ZERO ---
    window.essenzeDisponibili = magazzino.filter(p => 
        (p.formato === 'ml' || (p.categoria && p.categoria.toLowerCase().includes('materie'))) && 
        p.giacenza > 0
    );
...
window.disegnaListaEssenzeCassa = function (essenze) {
    ...
    essenze.forEach(ess => {
        ...
        div.style.borderLeft = '4px solid transparent'; // Predispone il bordo per l'hover

        div.innerHTML = `<b style="color: #ffffff; font-size: 1.8vh; transition: 0.2s;">${ess.descrizione}</b> <div style="font-size:1.6vh; color:#00cc66; margin-top: 5px;">Giacenza attuale: ${ess.giacenza} ml</div>`;

        // --- FORZATURA EFFETTO LUXURY ORO AL MOUSE OVER ---
        div.onmouseover = function () {
            this.style.backgroundColor = "rgba(191, 149, 63, 0.25)";
            this.style.borderLeft = "4px solid #bf953f";
            this.querySelector('b').style.color = "#fcf6ba"; // Colora di oro anche il testo!
        };
        div.onmouseout = function () {
            this.style.backgroundColor = "transparent";
            this.style.borderLeft = "4px solid transparent";
            this.querySelector('b').style.color = "#ffffff"; // Torna bianco
        };
...
window.confermaEssenzaSpina = function (essenzaScelta) {
    // 1. Creiamo un clone temporaneo del Formato 
    let prodottoVirtuale = JSON.parse(JSON.stringify(formatoSpinaInAttesa));

    // 2. Uniamo i nomi per lo scontrino e per l'etichetta
    ...
    // 3. Calcoliamo il costo del liquido al millilitro letto dalla scheda del Fusto (mag-prezzo-acq)
    ...
    // 🔥 3.5 MAGIA PER RICARICA LIBERA: Calcola il prezzo di vendita al volo moltiplicando i ml!
    if (prodottoVirtuale.is_ricarica_libera) {
        prodottoVirtuale.prezzo = costoLiquido; // Costo al ml * ml venduti!
    }

    // 4. Assembliamo la Distinta Base al volo per lo scarico automatico dei ml dal fusto
    if (!prodottoVirtuale.componenti_kit) prodottoVirtuale.componenti_kit = [];
    prodottoVirtuale.componenti_kit.push({ codice: essenzaScelta.codice, qta: prodottoVirtuale.volume_spina });

    prodottoVirtuale.is_kit = true;
    prodottoVirtuale.tipo_kit = 'DINAMICO';
    prodottoVirtuale.is_formato_spina = false; // Spegne il flag per evitare loop

    chiudiModale('modal-scegli-essenza');

    // 5. Lo invia alla cassa come se fosse un prodotto normalissimo, col suo prezzo perfetto!
...
// ==========================================
// 📥 IMPORTAZIONE MASSIVA DA EXCEL (Listino)
// ==========================================
window.importaExcelMagazzino = function (event) {
    ...
    reader.onload = function (e) {
        ...
        // Legge il file con SheetJS
        let workbook = XLSX.read(data, { type: 'array' });
        let primaPagina = workbook.SheetNames[0];
        let foglio = workbook.Sheets[primaPagina];

        // Converte il foglio in una griglia/matrice [riga][colonna]
        let righe = XLSX.utils.sheet_to_json(foglio, { header: 1 });

        if (righe.length <= 1) {
            mostraAvvisoModale("Il file Excel sembra vuoto o non contiene dati validi.");
            return;
        }

        let contatoreAggiunti = 0;
        let categorieDaAggiungere = new Set();

        let tx = db.transaction('magazzino', 'readwrite');
        let store = tx.objectStore('magazzino');

        // Parte da i = 1 per saltare l'intestazione della prima riga
        for (let i = 1; i < righe.length; i++) {
            let row = righe[i];

            // Salta righe vuote o senza il CODICE principale (Colonna A)
            if (!row || row.length === 0 || !row[0]) continue;

            // Estrazione dati di base
            let codice = String(row[0]).trim();                                       // Col. A (0)
            let categoria = row[1] ? String(row[1]).trim().toUpperCase() : "VARIE";   // Col. B (1)
            let descrizione = row[3] ? String(row[3]).trim().toUpperCase() : "NUOVO ARTICOLO"; // Col. D (3)
            let formato = row[4] ? String(row[4]).trim() : "";                        // Col. E (4)
            let fornitore = row[6] ? String(row[6]).trim().toUpperCase() : "";        // Col. G (6)

            // --- LETTURA ESSENZA DA COLONNA H (INDICE 7) ---
            let strEssenza = row[7] ? String(row[7]).trim().toLowerCase() : "no";
            let isEssenza = (strEssenza === "si" || strEssenza === "sì");

            // Pulizia e validazione del Prezzo di Acquisto (Costo Netto) (Assunto in Col. F / Indice 5)
            let rawPrezzoAcq = row[5];
            let prezzoAcq = 0;
            if (typeof rawPrezzoAcq === 'number') {
                prezzoAcq = rawPrezzoAcq;
            } else if (typeof rawPrezzoAcq === 'string') {
                // Sostituisce virgole con punti e rimuove simboli di valuta
                prezzoAcq = parseFloat(rawPrezzoAcq.replace('€', '').replace(',', '.').trim());
            }
            // Fallback di sicurezza in caso di NaN o valori non validi
            if (isNaN(prezzoAcq)) {
                prezzoAcq = 0;
            }

            // --- CONVERSIONE PREZZO ESSENZA (Da €/L a €/ml) ---
            if (isEssenza) {
                // Calcola il costo per singolo millilitro
                prezzoAcq = prezzoAcq / 1000;

                // Aggiunge la dicitura fusto madre per riconoscerlo
                if (!descrizione.includes("FUSTO")) {
                    descrizione = descrizione + " (FUSTO MADRE)";
                }
            }

            // Salviamo la categoria nel raccoglitore per crearla alla fine se non esiste
            if (categoria !== "") categorieDaAggiungere.add(categoria);

            // Composizione scheda prodotto per il Database
            let prodotto = {
                codice: codice,
                categoria: categoria,
                fornitore: fornitore,
                descrizione: descrizione,
                formato: isEssenza ? "ml" : formato,
                prezzoAcquisto: prezzoAcq,
                prezzo: 0,
                giacenza: 0,

                // 🔥 NUOVO: Scorta minima a 200 per le essenze, 0 per gli altri
                scorta_minima: isEssenza ? 200 : 0,

                is_formato_spina: false,
                note: isEssenza ? "Fragranza Madre convertita automaticamente in €/ml. Scorta minima impostata a 200ml." : "",
                iva: 22,
                tipo: "PZ"
            };
        ...
        tx.oncomplete = async function () {
            ...
            // Il modale rispetta la regola di evitare gli alert di sistema
            mostraAvvisoModale(`✅ Importazione completata con successo!<br>Sono state create o aggiornate <b>${contatoreAggiunti}</b> schede prodotto dal tuo listino.`);
        ...
        tx.onerror = function () {
            mostraAvvisoModale("❌ Si è verificato un errore durante il salvataggio dei prodotti nel database.");
        };
    };
...
// ==========================================
// 🔧 FIX 2: TASTO ESCI E PULIZIA RICERCA
// ==========================================
const btnEsciCassa = document.querySelector('.tasto-fisico img[src*="esci.png"]');
if (btnEsciCassa && btnEsciCassa.parentElement) {
    btnEsciCassa.parentElement.onclick = function () {
        // 1. Svuota il campo di ricerca testuale (Nome Prodotto)
        let inputCerca = document.getElementById('cerca-prodotto');
        if (inputCerca) inputCerca.value = '';

        // 2. Svuota il campo del Codice a Barre
        let inputBarcode = document.getElementById('campo-barcode');
        if (inputBarcode) inputBarcode.value = '';

        // 3. Nasconde ESATTAMENTE solo le tendine (senza far sparire i contenitori)
        document.querySelectorAll('.lista-suggerimenti, .autocomplete-items, #lista-ricerca, #ord-lista-ricerca').forEach(el => {
...
// 4. Nasconde la tendina anche se clicchi fuori (sul vuoto dello schermo)
...
// ==========================================
// 🔧 FIX 1: FORZA CARICAMENTO MAGAZZINO ALL'AVVIO
// ==========================================
window.addEventListener('load', () => {
    ...
    }, 1000); // Attende l'apertura del database e popola la lista
});

// ==========================================
// 🚚 AUTOPILOTA: LISTA FORNITORI IN ORDINI
// ==========================================
window.popolaListaFornitori = function () {
    if (!db) return;
    let tx = db.transaction('magazzino', 'readonly');
    let store = tx.objectStore('magazzino');
    let req = store.getAll();
    ...
    req.onsuccess = function () {
        let fornitori = new Set();

        // Estrapola tutti i fornitori unici dal magazzino
        req.result.forEach(prod => {
            if (prod.fornitore && prod.fornitore.trim() !== "") {
                fornitori.add(prod.fornitore.trim().toUpperCase());
            }
        });

        // Popola la tendina invisibile (datalist)
        let datalist = document.getElementById('lista-fornitori');
        if (datalist) {
            datalist.innerHTML = "";

            // Ordina alfabeticamente e crea le opzioni
            Array.from(fornitori).sort().forEach(f => {
                let opt = document.createElement('option');
                opt.value = f;
...
// ==========================================
// 🎯 FOCUS AUTOMATICO SUL CAMPO FORNITORE
// ==========================================
const modalDettaglioOrdine = document.getElementById('modal-dettaglio-ordine');
if (modalDettaglioOrdine) {
    // L'Observer controlla se il modale diventa visibile
    new MutationObserver((mutations) => {
        for (let m of mutations) {
            if (m.attributeName === 'style' && modalDettaglioOrdine.style.display !== 'none') {
                // Aspetta un istante per l'animazione di apertura e poi piazza il cursore
                setTimeout(() => {
                    let inputFornitore = document.getElementById('ord-fornitore');
                    if (inputFornitore) {
                        inputFornitore.focus();
                        popolaListaFornitori(); // Carica la lista in anteprima!
                    }
                }, 150);
            }
        }
    }).observe(modalDettaglioOrdine, { attributes: true });
}

// ==========================================
// 🎯 NAVIGAZIONE TASTIERA: RICERCA ORDINI
// ==========================================
let indiceFocusOrdini = -1;

const inputCercaOrdini = document.getElementById('ord-cerca-articolo');
if (inputCercaOrdini) {
    // Azzera l'indice ogni volta che scrivi una nuova lettera
    inputCercaOrdini.addEventListener('input', function () {
        indiceFocusOrdini = -1;
    });

    inputCercaOrdini.addEventListener('keydown', function (e) {
        let tendina = document.getElementById('ord-lista-ricerca');

        // Se la tendina è chiusa o inesistente, ignora i comandi
        if (!tendina || tendina.style.display === 'none') return;

        // Prende tutti i suggerimenti cliccabili dentro la tendina
        let elementi = tendina.querySelectorAll('div');
        if (elementi.length === 0) return;

        if (e.key === 'ArrowDown') {
            e.preventDefault(); // Evita che il cursore di testo si muova
            indiceFocusOrdini++;
            evidenziaElementoOrdini(elementi);
        } else if (e.key === 'ArrowUp') {
            e.preventDefault();
            indiceFocusOrdini--;
            evidenziaElementoOrdini(elementi);
            ...
            e.preventDefault();
            // Se c'è un elemento selezionato, simula un clic
            if (indiceFocusOrdini > -1 && elementi[indiceFocusOrdini]) {
                elementi[indiceFocusOrdini].click();
            }
        }
    });
}

function evidenziaElementoOrdini(elementi) {
    // 1. Pulisce lo stile di tutti gli elementi per resettarli
    elementi.forEach(el => {
        el.style.backgroundColor = '';
        el.style.color = '';
        el.style.borderLeft = '';
    ...
    // 2. Crea un "loop" infinito (se vai oltre la fine, torni al primo)
    if (indiceFocusOrdini >= elementi.length) indiceFocusOrdini = 0;
    if (indiceFocusOrdini < 0) indiceFocusOrdini = elementi.length - 1;

    // 3. Applica lo stile Luxury all'elemento attualmente evidenziato
    let elementoAttivo = elementi[indiceFocusOrdini];
    if (elementoAttivo) {
        elementoAttivo.style.backgroundColor = 'rgba(77, 136, 255, 0.3)';
        elementoAttivo.style.color = '#ffffff';
        elementoAttivo.style.borderLeft = '4px solid #4d88ff';

        // Fa scorrere automaticamente la barra di scorrimento se l'elemento esce dallo schermo
        elementoAttivo.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
    }
}

// ==========================================
// 🗑️ RIMUOVI ARTICOLO DALL'ORDINE
// ==========================================
window.rimuoviArticoloOrdine = function (indice) {
    if (!ordineApertoAttuale || !ordineApertoAttuale.articoli) return;

    // Rimuove esattamente l'articolo cliccato dalla memoria dell'ordine
    ordineApertoAttuale.articoli.splice(indice, 1);

    // Ridisegna la tabella aggiornata e ricalcola il totale
    disegnaArticoliOrdine();
};

// ==========================================
// 💧 SELEZIONE RAPIDA ESSENZE IN CASSA
// ==========================================

window.apriSceltaEssenze = function () {
    document.getElementById('step-essenza-categoria').style.display = 'block';
    document.getElementById('step-essenza-formato').style.display = 'none';
    apriModale('modal-essenze');
};

window.indietroCategoriaEssenza = function () {
    document.getElementById('step-essenza-categoria').style.display = 'block';
    document.getElementById('step-essenza-formato').style.display = 'none';
};

window.mostraFormatiEssenza = function (prefisso, nomeCategoria) {
    document.getElementById('lbl-categoria-scelta').textContent = nomeCategoria;

    let griglia = document.getElementById('griglia-formati-essenza');
    griglia.innerHTML = '';

    let formati = [15, 30, 50, 100];

    formati.forEach(ml => {
        let codiceFinale = prefisso + ml;
        let btn = document.createElement('button');
        btn.className = "btn-modal";
        btn.style.cssText = "border: 1px solid #4d88ff; color: #ffffff; background: rgba(77, 136, 255, 0.15); padding: 20px; font-size: 20px; font-weight: bold; border-radius: 6px; cursor: pointer; transition: 0.2s;";
        btn.onmouseover = () => btn.style.background = "rgba(77, 136, 255, 0.4)";
        btn.onmouseout = () => btn.style.background = "rgba(77, 136, 255, 0.15)";
        btn.textContent = ml + " ml";

        btn.onclick = () => inviaCodiceScannerEssenza(codiceFinale);
        griglia.appendChild(btn);
    });

    document.getElementById('step-essenza-categoria').style.display = 'none';
    document.getElementById('step-essenza-formato').style.display = 'block';
};

window.inviaCodiceScannerEssenza = async function (codice) {
    let codiceRicerca = codice.trim().toUpperCase();

    try {
        // Cerca il prodotto direttamente nel DB in modo istantaneo
        const magazzinoGrezzo = await getAll('magazzino');
        const magazzinoCompleto = typeof calcolaGiacenzeVirtualiBOM === "function" ? calcolaGiacenzeVirtualiBOM(magazzinoGrezzo) : magazzinoGrezzo;

        const prodotto = magazzinoCompleto.find(p => String(p.codice).toUpperCase() === codiceRicerca);

        if (prodotto) {
            // ✅ FORMATO TROVATO: Chiudiamo la scelta essenze in modo fluido e mettiamo in cassa
            chiudiModale('modal-essenze');

            if (typeof aggiungiProdotto === "function") {
                aggiungiProdotto(prodotto);
            }
        } else {
            // ❌ FORMATO NON TROVATO: Apriamo l'avviso MA LASCIAMO APERTO il modale essenze sotto
            // In questo modo blocchiamo totalmente lo sfarfallio della cassa!
            mostraAvvisoModale("FORMATO NON DISPONIBILE O ESAURITO A MAGAZZINO");
        }
    } catch (e) {
        console.error("Errore ricerca essenza:", e);
        mostraAvvisoModale("ERRORE DURANTE LA RICERCA. FORMATO NON DISPONIBILE.");
    }
};

// ==========================================
// 🎨 FIX 3 e 4 DEFINITIVO: HOVER ESTESO ALLE TENDINE
// ==========================================
setTimeout(() => {
    // 1. Disintegriamo la vecchia regola CSS che causava il verde acceso al focus
    let stileOverride = document.createElement('style');
    stileOverride.innerHTML = `
        #campo-barcode:focus, 
        #campo-scheda:focus, 
        #campo-pagamento:focus, 
        #campo-sconto:focus, 
        #cassa-voucher-input:focus
    `;
    document.head.appendChild(stileOverride);

    // 2. Riapplichiamo la logica intelligente per il verde
    const campiCassa = ['campo-barcode', 'campo-scheda', 'campo-pagamento', 'campo-sconto', 'cassa-voucher-input'];
    
    campiCassa.forEach(id => {
        let campo = document.getElementById(id);
        if (campo) {
            // 🔥 LOGICA CHIAVE: Per Sconto e Pagamento, "ascoltiamo" l'intero div genitore che contiene anche la lista.
            // Per gli altri campi singoli, ascoltiamo solo la casella di testo.
            let targetHover = (id === 'campo-pagamento' || id === 'campo-sconto') && campo.parentElement ? campo.parentElement : campo;
            
            targetHover.addEventListener('mouseenter', function() {
                campo.style.setProperty('background-color', '#ccde57', 'important'); 
                
                if (id === 'cassa-voucher-input') {
                    campo.focus();
                }
            });
            
            targetHover.addEventListener('mouseleave', function() {
...
// ==========================================
// 💧 NUOVO MOTORE DINAMICO LINEA ESSENZE E RICARICHE
// ==========================================
let tipoEssenzaAttivo = '';
let categoriaEssenzaAttiva = '';

// Diamo in pasto al sistema i codici di partenza che avevamo stabilito
let configEssenzeDefault = {
    VENDITA: {
        "Classic": { 15: "CLA-15", 30: "CLA-30", 50: "CLA-50", 100: "CLA-100" },
        "Nicchia": { 15: "NIC-15", 30: "NIC-30", 50: "NIC-50", 100: "NIC-100" },
        "Super Nicchia": { 15: "SNIC-15", 30: "SNIC-30", 50: "SNIC-50", 100: "SNIC-100" }
    },
    RICARICA: {
        "Classic": { 15: "RIC-CLA-15", 30: "RIC-CLA-30", 50: "RIC-CLA-50", 100: "RIC-CLA-100" },
        "Nicchia": { 15: "RIC-NIC-15", 30: "RIC-NIC-30", 50: "RIC-NIC-50", 100: "RIC-NIC-100" },
        "Super Nicchia": { 15: "RIC-SNIC-15", 30: "RIC-SNIC-30", 50: "RIC-SNIC-50", 100: "RIC-SNIC-100" }
    }
};

function getConfigEssenze() {
    let saved = localStorage.getItem('gestionale_codici_essenze');
    return saved ? JSON.parse(saved) : configEssenzeDefault;
}

// Override della funzione di apertura per farle mostrare il nuovo "Step 1"
window.apriSceltaEssenze = function() {
    document.getElementById('step-essenza-tipo').style.display = 'block';
    document.getElementById('step-essenza-categoria').style.display = 'none';
    document.getElementById('step-essenza-formato').style.display = 'none';
    apriModale('modal-essenze');
};

window.scegliTipoEssenza = function(tipo) {
    tipoEssenzaAttivo = tipo;
    let icona = tipo === 'VENDITA' ? '🛍️ NUOVO FLACONE' : '♻️ RICARICA';
    document.getElementById('lbl-tipo-scelto').textContent = icona;
    
    document.getElementById('step-essenza-tipo').style.display = 'none';
    document.getElementById('step-essenza-categoria').style.display = 'block';
    document.getElementById('step-essenza-formato').style.display = 'none';
};

window.indietroTipoEssenza = function() {
    document.getElementById('step-essenza-tipo').style.display = 'block';
    document.getElementById('step-essenza-categoria').style.display = 'none';
    document.getElementById('step-essenza-formato').style.display = 'none';
};

window.scegliCategoriaEssenza = function(categoria) {
    categoriaEssenzaAttiva = categoria;
    
    let tipoSceltoTesto = tipoEssenzaAttivo === 'VENDITA' ? 'Nuovo Flacone' : 'Ricarica';
    document.getElementById('lbl-categoria-scelta').textContent = `${tipoSceltoTesto} - ${categoria.toUpperCase()}`;

    // Popola dinamicamente la griglia dei formati leggendola dal DB
    let griglia = document.getElementById('griglia-formati-essenza');
    griglia.innerHTML = '';
    
    let config = getConfigEssenze();
    let codiciFormati = config[tipoEssenzaAttivo][categoria];
    
    if(!codiciFormati) {
        mostraAvvisoModale("Nessun formato configurato per questa linea.");
        return;
    }

    let formati = Object.keys(codiciFormati).sort((a,b) => parseInt(a) - parseInt(b));
    
    formati.forEach(ml => {
        let codice = codiciFormati[ml];
        griglia.innerHTML += `
            <button class="btn-modal" style="background: rgba(0, 51, 102, 0.4); border: 1px solid #4d88ff; color: #b3d9ff; padding: 20px; font-size: 24px; font-weight: bold; transition: 0.2s;" 
                    onmouseover="this.style.background='rgba(77, 136, 255, 0.3)'" 
                    onmouseout="this.style.background='rgba(0, 51, 102, 0.4)'" 
                    onclick="inviaCodiceScannerEssenza('${codice}')">${ml} ml</button>
        `;
    });
...
// Costruzione Modale Impostazioni
window.apriImpostazioniEssenze = function() {
    let config = getConfigEssenze();
    let html = '';
    
    ['VENDITA', 'RICARICA'].forEach(tipo => {
        // 🔥 FIX: Aggiunto "height: max-content;" per forzare lo sfondo ad avvolgere tutti i campi fino in fondo
        html += `<div style="flex: 1; background: rgba(255,255,255,0.05); padding: 15px; border-radius: 8px; height: max-content; margin-bottom: 15px;">
                    <div style="color: ${tipo === 'VENDITA' ? '#00ffcc' : '#fcf6ba'}; font-weight: bold; font-size: 18px; margin-bottom: 15px; border-bottom: 1px solid rgba(255,255,255,0.1); padding-bottom: 5px;">${tipo === 'VENDITA' ? '🛍️ VENDITA NUOVA' : '♻️ RICARICHE'}</div>`;
        
        ['Classic', 'Nicchia', 'Super Nicchia'].forEach(cat => {
            html += `<div style="color: #fff; margin-bottom: 10px; margin-top: 15px; font-weight: bold;">${cat.toUpperCase()}</div>`;
            [15, 30, 50, 100].forEach(ml => {
                let currentCode = config[tipo][cat] && config[tipo][cat][ml] ? config[tipo][cat][ml] : "";
                html += `
                    <div style="display: flex; align-items: center; margin-bottom: 5px; gap: 10px;">
                        <span style="color: #888; width: 70px; white-space: nowrap; text-align: right;">${ml} ml:</span>
                        <input type="text" id="cfg-${tipo}-${cat.replace(/ /g,'')}-${ml}" class="luxury-crm-input" style="margin: 0; width: 140px; height: 30px; font-size: 14px; text-transform: uppercase; color: #b3d9ff;" value="${currentCode}">
                    </div>
                `;
            });
        });
    ...
    document.getElementById('contenitore-config-essenze').innerHTML = html;
    apriModale('modal-impostazioni-essenze');
};

// Salvataggio Impostazioni
window.salvaImpostazioniEssenze = function() {
    let config = getConfigEssenze();
    
    ['VENDITA', 'RICARICA'].forEach(tipo => {
        ['Classic', 'Nicchia', 'Super Nicchia'].forEach(cat => {
            if(!config[tipo][cat]) config[tipo][cat] = {};
            [15, 30, 50, 100].forEach(ml => {
                let inputElement = document.getElementById(`cfg-${tipo}-${cat.replace(/ /g,'')}-${ml}`);
                if(inputElement) {
                    let val = inputElement.value.trim().toUpperCase();
                    if(val) config[tipo][cat][ml] = val;
                }
            });
        });
    });
    
    localStorage.setItem('gestionale_codici_essenze', JSON.stringify(config));
    chiudiModale('modal-impostazioni-essenze');
    mostraAvvisoModale("Codici salvati in memoria con successo!");
};

// ==========================================
// 🛑 FIX: ALLINEAMENTO AVVISI AI 3 STEP
// ==========================================
window.chiudiModaleAvviso = function () {
    let testoAvviso = (document.getElementById('msg-avviso').innerText || "").toUpperCase();
    let isClienteNonTrovato = testoAvviso.includes("NESSUN CLIENTE TROVATO");
    let isProdottoMancante = testoAvviso.includes("DISPONIBIL") || testoAvviso.includes("ESAURIT") || testoAvviso.includes("GIACENZA");
    let modaleBlocco = document.getElementById('modal-sblocco-cassa');

    if (isProdottoMancante) {
        // Se il prodotto manca, non ripartiamo da zero: apriamo il modale direttamente sul formato scelto!
        if (typeof apriModale === "function") apriModale('modal-essenze');
        else document.getElementById('modal-essenze').style.display = 'block';
        
        let stepTipo = document.getElementById('step-essenza-tipo');
        let stepCat = document.getElementById('step-essenza-categoria');
        let stepFormato = document.getElementById('step-essenza-formato');
        
        if (stepTipo) stepTipo.style.display = 'none';
        if (stepCat) stepCat.style.display = 'none';
        if (stepFormato) stepFormato.style.display = 'block';
        ...
        if (modaleBlocco && modaleBlocco.style.display !== 'none') {
            ...
            setTimeout(() => { let campoScheda = document.getElementById('campo-scheda'); if (campoScheda) { campoScheda.focus(); campoScheda.select(); } }, 100);
        } else {
            let campoBarcode = document.getElementById('campo-barcode');
            let modaleMenu = document.getElementById('modal-menu-principale');
            if (campoBarcode && !campoBarcode.disabled && modaleMenu && modaleMenu.style.display === 'none') { setTimeout(() => campoBarcode.focus(), 100); }
        }
    }
};

// ==========================================
// 📥 IMPORTAZIONE ORDINE DA EXCEL
// ==========================================
window.importaOrdineExcel = function (event) {
    let file = event.target.files[0];
    if (!file) return;
    ...
    reader.onload = async function (e) {
        try {
            ...
            // Recuperiamo tutto il magazzino per fare il matching dei prodotti
            let magazzinoCompleto = await getAll('magazzino');
            let articoliImportati = [];
            let fornitoreStimato = "FORNITORE DA DEFINIRE";
            let prodottiSconosciuti = 0;

            // Iniziamo dalla riga 1 (saltando l'intestazione)
            for (let i = 1; i < righe.length; i++) {
                let row = righe[i];
                if (!row || row.length === 0 || !row[0]) continue;

                // STRUTTURA ATTESA: Col. A = Codice, Col. B = Quantità (Opzionale: Col. C = Descrizione)
                let codice = String(row[0]).trim();
                let qta = parseInt(row[1]) || 1; // Se non c'è quantità, metti 1 di default

                // Cerca nel magazzino
                let prodottoDB = magazzinoCompleto.find(p => p.codice === codice);

                if (prodottoDB) {
                    if (prodottoDB.fornitore && fornitoreStimato === "FORNITORE DA DEFINIRE") {
                        fornitoreStimato = prodottoDB.fornitore.toUpperCase();
                    }
                    articoliImportati.push({
                        codice: prodottoDB.codice,
                        descrizione: prodottoDB.descrizione,
                        prezzoAcquisto: prodottoDB.prezzoAcquisto || 0,
                        qta_ordinata: qta,
                        qta_ricevuta: 0
                    });
                } else {
                    // Prodotto non presente in magazzino (lo inserisce con prezzo zero)
                    articoliImportati.push({
                        codice: codice,
                        descrizione: row[2] ? String(row[2]).trim().toUpperCase() : "ARTICOLO SCONOSCIUTO",
                        prezzoAcquisto: 0,
                        qta_ordinata: qta,
                        qta_ricevuta: 0
                    });
            ...
            if (articoliImportati.length === 0) {
                mostraAvvisoModale("Nessun articolo valido trovato nel file Excel.");
                event.target.value = '';
                return;
            }
            ...
            let nuovoOrdine = {
                id_ordine: Date.now(),
                data: getOggiString(),
                fornitore: fornitoreStimato,
                stato: 'BOZZA',
                articoli: articoliImportati,
                spese_spedizione: 0
            };
            ...
            tx.oncomplete = function () {
                let msg = `Ordine caricato con successo!<br>Sono stati inseriti <b>${articoliImportati.length}</b> articoli.`;
                if (prodottiSconosciuti > 0) {
                    msg += `<br><br><span style="color:#ffcc00">Attenzione: ${prodottiSconosciuti} articoli non sono presenti nel tuo magazzino (avranno prezzo d'acquisto pari a zero).</span>`;
                }
                
                mostraAvvisoModale(msg);
                event.target.value = ''; // Pulisce l'input
                caricaListaOrdini();
                apriDettaglioOrdine(nuovoOrdine.id_ordine); // Apre subito l'ordine
                if (typeof salvaOrdineCloud === "function") salvaOrdineCloud(nuovoOrdine);
            };

        } catch (error) {
            mostraAvvisoModale("❌ Errore durante l'importazione. Assicurati che il file sia un Excel formattato correttamente (Colonna 1: Codice, Colonna 2: Quantità).");
            console.error(error);
            event.target.value = '';
        }
    };
...
// ==========================================
// 😃 INSERIMENTO EMOJI NELL'EDITOR WHATSAPP
// ==========================================
window.inserisciTestoWA = function(testoDaInserire) {
    // Ora l'ID è quello corretto letto dal tuo file!
    let txtArea = document.getElementById('editor-messaggio-textarea'); 
    
    if (!txtArea) {
        console.error("Errore: Casella di testo non trovata!");
        return;
    }

    // Catturiamo la posizione esatta del cursore
    let startPos = txtArea.selectionStart;
    let endPos = txtArea.selectionEnd;

    // Spezziamo il testo in due e inseriamo l'emoji in mezzo
    let testoPrima = txtArea.value.substring(0, startPos);
    let testoDopo = txtArea.value.substring(endPos, txtArea.value.length);
    
    txtArea.value = testoPrima + testoDaInserire + testoDopo;

    // Rimettiamo il focus sulla casella e spostiamo il cursore subito dopo l'emoji
    txtArea.focus();
    txtArea.selectionStart = startPos + testoDaInserire.length;
    txtArea.selectionEnd = startPos + testoDaInserire.length;
};

// ==========================================
// ⚖️ LOGICA RICARICA LIBERA A PESO/ML
// ==========================================
window.apriModaleRicaricaLibera = function() {
    document.getElementById('input-ml-ricarica-libera').value = '';
    chiudiModale('modal-essenze'); // Chiude la scelta Vendita/Ricarica
    apriModale('modal-ricarica-libera');
    setTimeout(() => document.getElementById('input-ml-ricarica-libera').focus(), 100);
};

window.procediRicaricaLibera = function() {
    let mlStr = document.getElementById('input-ml-ricarica-libera').value;
    let ml = parseInt(mlStr);
    
    if (isNaN(ml) || ml <= 0) {
        mostraAvvisoModale("Inserisci una quantità valida in millilitri.");
        return;
    }
    
    chiudiModale('modal-ricarica-libera');
    
    // Crea un "Formato Virtuale" fittizio passandogli i millilitri esatti!
    let formatoLibero = {
        codice: "RIC-LIB-" + ml, // Es. RIC-LIB-23
        descrizione: `RICARICA ${ml}ml`,
        categoria: "RICARICHE",
        is_virtuale: true,
        is_formato_spina: true, // Questo flag attiva la scelta essenza
        volume_spina: ml,
        is_ricarica_libera: true, // FLAG SPECIALE PER IL CALCOLO DEL PREZZO AL ML!
        is_kit: false,
        componenti_kit: []
    };
    
    // Apre la finestra delle essenze passandogli il formato speciale
    apriModaleScegliEssenza(formatoLibero);
};

// ==========================================
// 🖨️ LOGICA REGISTRATORE FISCALE (RT)
// ==========================================
window.apriImpostazioniRT = function() {
    chiudiModale('modal-impostazioni-menu');
    
    // Carica i dati salvati
    let ipSalvato = localStorage.getItem('rt_indirizzo_ip') || '';
    let marcaSalvata = localStorage.getItem('rt_marca') || 'ALTRO';
    
    document.getElementById('rt-ip').value = ipSalvato;
    document.getElementById('rt-marca').value = marcaSalvata;
    
    apriModale('modal-impostazioni-rt');
};

window.salvaImpostazioniRT = function() {
    let ip = document.getElementById('rt-ip').value.trim();
    let marca = document.getElementById('rt-marca').value;
    
    localStorage.setItem('rt_indirizzo_ip', ip);
    localStorage.setItem('rt_marca', marca);
    
    chiudiModale('modal-impostazioni-rt');
    apriModale('modal-impostazioni-menu');
    mostraAvvisoModale("✅ Configurazione Fiscale Salvata!<br><br>Il gestionale è pronto per inviare gli scontrini all'indirizzo IP: " + (ip || "Non inserito"));
};

// Funzione master che compila e invia lo scontrino
// Funzione master che compila e invia lo scontrino
window.inviaScontrinoFiscale = async function(carrello, totaleScontrino, scontoApplicato = 0, pagatoContanti = 0, pagatoPOS = 0) {
    let ip = localStorage.getItem('rt_indirizzo_ip');
    let marca = localStorage.getItem('rt_marca'); 
    
    if (!ip) {
        console.warn("Nessun IP configurato. Lo scontrino è stato salvato solo nel gestionale.");
        return false;
    }
    
    mostraAvvisoModale("⏳ Trasmissione scontrino alla cassa...");

    if (marca === "CUSTOM" || marca === "ALTRO") {
        try {
            // 👇 INSERISCI QUI LE CREDENZIALI CHE HAI USATO PER ENTRARE NEL MENU
            let utenteCassa = "1BENI089756"; 
            let passwordCassa = "1BENI089756"; 
            
            // Crea il pass speciale criptato (Basic Auth)
            let tokenAuth = btoa(utenteCassa + ":" + passwordCassa);

            // 1. Costruiamo la "Ricetta" XML dello scontrino
            let xmlString = '<?xml version="1.0" encoding="utf-8"?>\n';
            xmlString += '<printer>\n';
            xmlString += '  <printRecMessage operator="1"/>\n'; 

            for (let item of carrello) {
                let prezzoVal = (item.prezzo || 0).toFixed(2);
                let qtaVal = item.qta || 1;
                let descTesto = item.descrizione.substring(0, 20); 
                xmlString += `  <printRecItem description="${descTesto}" quantity="${qtaVal}" amount="${prezzoVal}" department="1"/>\n`;
            }

            if (scontoApplicato > 0) {
                let scontoVal = scontoApplicato.toFixed(2);
                xmlString += `  <printRecSubtotal discountAmount="${scontoVal}" type="0"/>\n`;
            }

            if (pagatoContanti > 0 && pagatoPOS === 0) {
                xmlString += '  <printRecTotal description="CONTANTI" paymentType="0"/>\n';
            } else if (pagatoPOS > 0 && pagatoContanti === 0) {
                xmlString += '  <printRecTotal description="PAGAMENTO ELETTRONICO" paymentType="2"/>\n';
            } else if (pagatoContanti > 0 && pagatoPOS > 0) {
                xmlString += `  <printRecTotal description="CONTANTI" paymentType="0" amount="${pagatoContanti.toFixed(2)}"/>\n`;
                xmlString += `  <printRecTotal description="PAGAMENTO ELETTRONICO" paymentType="2" amount="${pagatoPOS.toFixed(2)}"/>\n`;
            } else {
                xmlString += '  <printRecTotal/>\n';
            }

            xmlString += '</printer>';

            // 5. Invio tramite Rete LAN con AUTENTICAZIONE
            let urlCassa = `http://${ip}/cgi-bin/fprint.cgi`;
            
            const response = await fetch(urlCassa, {
                method: 'POST',
                headers: { 
                    'Content-Type': 'text/xml',
                    'Authorization': 'Basic ' + tokenAuth  // <--- ECCO IL NOSTRO PASS!
                },
                body: xmlString
            });

            if (response.ok) {
                mostraAvvisoModale("✅ Scontrino stampato con successo!");
                return true;
            } else {
                // Se la cassa risponde male (es. rotolo finito o errore 401 per password errata)
                mostraAvvisoModale("❌ Errore dalla Cassa. Verifica il rotolo o la password inserita nel codice.");
                return false;
            }

        } catch (error) {
            console.error("Errore di comunicazione:", error);
            mostraAvvisoModale(`❌ Errore di connessione.<br>Assicurati di usare il <b>Live Server</b> e che l'IP sia corretto.`);
            return false;
        }
    } else {
        mostraAvvisoModale("❌ Protocollo non ancora supportato per questa marca.");
        return false;
    }
};
